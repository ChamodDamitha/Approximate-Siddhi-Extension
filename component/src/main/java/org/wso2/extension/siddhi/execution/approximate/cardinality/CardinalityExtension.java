...
@Extension(
        ...
        parameters = {
                ...
                @Parameter(
                        name = "accuracy",
                        description = "this is the accuracy for which the value is obtained",
        ...
        returnAttributes = {
                @ReturnAttribute(
                        name = "cardinality",
                        description = "Represents the cardinality at the event arrival",
                        type = {DataType.LONG}
                )
        },
        examples = {
                @Example(
                        ...
                        description = "cardinality is calculated for an accuracy of 0.01"
                ),
        }
)
...
//       validate number of attributes
        if (attributeExpressionExecutors.length != 2) {
            throw new SiddhiAppCreationException("2 attributes are expected but " +
                    attributeExpressionExecutors.length + " attributes are found");
        }

        //expressionExecutors[1] --> accurracy
        if (!(attributeExpressionExecutors[1] instanceof ConstantExpressionExecutor)) {
            throw new SiddhiAppCreationException("accurracy has to be a constant but found " +
        ...
        if (attributeExpressionExecutors[1].getReturnType() == Attribute.Type.DOUBLE) {
            ...
            throw new SiddhiAppCreationException("accurracy should be of type Double but found " +
        ...
        //validating the attribute to be variables
